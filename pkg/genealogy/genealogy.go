...
func (tg *TreeGenealogical) directRelationDescription(relative *person.Person, persons []*person.Person) string {
	if relative == nil {
		return ""
	}
...
func findPerson(ID string, persons []*person.Person) *person.Person {
	for _, person := range persons {
		if person.ID == ID {
			return person
		}
